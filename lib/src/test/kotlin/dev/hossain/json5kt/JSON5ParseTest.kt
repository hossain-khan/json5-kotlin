...
class JSON5ParseTest {
    ...
    fun `should parse empty object`() {
        JSON5.parse("{}").toAny() shouldBe emptyMap<String, Any?>()
    }
    ...
    fun `should parse simple object with string value`() {
        JSON5.parse("""{"key": "value"}""").toAny() shouldBe mapOf("key" to "value")
    }
    ...
    fun `should parse simple object with number value`() {
        JSON5.parse("""{"key": 42}""").toAny() shouldBe mapOf("key" to 42.0)
    }
    ...
    fun `should parse simple object with boolean value`() {
        JSON5.parse("""{"key": true}""").toAny() shouldBe mapOf("key" to true)
    }
    ...
    fun `should parse simple object with null value`() {
        JSON5.parse("""{"key": null}""").toAny() shouldBe mapOf("key" to null)
    }
    ...
    fun `should parse empty array`() {
        JSON5.parse("[]").toAny() shouldBe emptyList<Any?>()
    }
    ...
    fun `should parse array with values`() {
        JSON5.parse("""[1, "string", true, null]""").toAny() shouldBe listOf(1.0, "string", true, null)
    }
    ...
    fun `should parse double quoted string property names`() {
        JSON5.parse("""{"a":1}""").toAny() shouldBe mapOf("a" to 1.0)
    }
    ...
    fun `should parse unicode property names`() {
        JSON5.parse("""{ùńîċõďë:9}""").toAny() shouldBe mapOf("ùńîċõďë" to 9.0)
    }
    ...
    fun `should preserve __proto__ property names`() {
        val result = JSON5.parse("""{"__proto__":1}""").toAny() as Map<*, *>
    ...
    fun `should parse multiple properties`() {
        JSON5.parse("""{abc:1,def:2}""").toAny() shouldBe mapOf("abc" to 1.0, "def" to 2.0)
    }
    ...
    fun `should parse nested objects`() {
        JSON5.parse("""{a:{b:2}}""").toAny() shouldBe mapOf("a" to mapOf("b" to 2.0))
    }
    ...
    fun `should parse multiple array values`() {
        JSON5.parse("[1,2]").toAny() shouldBe listOf(1.0, 2.0)
    }
    ...
    fun `should parse nested arrays`() {
        JSON5.parse("[1,[2,3]]").toAny() shouldBe listOf(1.0, listOf(2.0, 3.0))
    }
    ...
    fun `should parse leading zeroes`() {
        JSON5.parse("[0,0.,0e0]").toAny() shouldBe listOf(0.0, 0.0, 0.0)
    }
    ...
    fun `should parse integers`() {
        JSON5.parse("[1,23,456,7890]").toAny() shouldBe listOf(1.0, 23.0, 456.0, 7890.0)
    }
    ...
    fun `should parse signed numbers`() {
        JSON5.parse("[-1,+2,-.1,-0]").toAny() shouldBe listOf(-1.0, 2.0, -0.1, -0.0)
    }
    ...
    fun `should parse leading decimal points`() {
        JSON5.parse("[.1,.23]").toAny() shouldBe listOf(0.1, 0.23)
    }
    ...
    fun `should parse fractional numbers`() {
        JSON5.parse("[1.0,1.23]").toAny() shouldBe listOf(1.0, 1.23)
    }
    ...
    fun `should parse exponents`() {
        JSON5.parse("[1e0,1e1,1e01,1.e0,1.1e0,1e-1,1e+1]").toAny() shouldBe
    ...
    fun `should parse hexadecimal numbers`() {
        JSON5.parse("[0x1,0x10,0xff,0xFF]").toAny() shouldBe listOf(1.0, 16.0, 255.0, 255.0)
    }
    ...
    fun `should parse infinity values`() {
        JSON5.parse("[Infinity,-Infinity]").toAny() shouldBe listOf(Double.POSITIVE_INFINITY, Double.NEGATIVE_INFINITY)
    }
    ...
    fun `should parse NaN`() {
        val result = JSON5.parse("NaN").toAny()
    ...
    fun `should parse bare numbers`() {
        JSON5.parse("1").toAny() shouldBe 1.0
        JSON5.parse("+1.23e100").toAny() shouldBe 1.23e100
    }
    ...
    fun `should parse bare hexadecimal numbers`() {
        JSON5.parse("0x1").toAny() shouldBe 1.0
        ...
        JSON5.parse("-0x0123456789abcdefABCDEF").toAny() shouldBe -1.3754889325393114E24
    }
    ...
    fun `should parse double quoted strings`() {
        JSON5.parse("\"abc\"").toAny() shouldBe "abc"
    }
    ...
    fun `should parse quotes in strings`() {
        JSON5.parse("""['"',"'"]""").toAny() shouldBe listOf("\"", "'")
    }
    ...
    fun `should parse single-line comments`() {
        JSON5.parse("{//comment\n}").toAny() shouldBe emptyMap<String, Any?>()
    }
    ...
    fun `should parse single-line comments at end of input`() {
        JSON5.parse("{}//comment").toAny() shouldBe emptyMap<String, Any?>()
    }
    ...
    fun `should parse whitespace`() {
        JSON5.parse("{\t\u000B\u000C \u00A0\uFEFF\n\r\u2028\u2029\u2003}").toAny() shouldBe emptyMap<String, Any?>()
    }
    ...
    fun `should modify property values using reviver`() {
        JSON5.parse("{a:1,b:2}") { k, v -> if (k == "a") "revived" else v }.toAny() shouldBe mapOf("a" to "revived", "b" to 2.0)
    }
    ...
    fun `parse diverse identifiers`() {
        JSON5.parse("{ _: 1 }").toAny() shouldBe mapOf("_" to 1.0)
        JSON5.parse("{ \$: 2 }").toAny() shouldBe mapOf("\$" to 2.0)
        JSON5.parse("{ _ident: 3 }").toAny() shouldBe mapOf("_ident" to 3.0)
        JSON5.parse("{ \$ident: 4 }").toAny() shouldBe mapOf("\$ident" to 4.0)
        JSON5.parse("{ ident_: 5 }").toAny() shouldBe mapOf("ident_" to 5.0)
        JSON5.parse("{ ident\$: 6 }").toAny() shouldBe mapOf("ident\$" to 6.0)
        JSON5.parse("{ üñîçødé: 7 }").toAny() shouldBe mapOf("üñîçødé" to 7.0) // Already covered but good to have
        JSON5.parse("{ \\u0061b\\u0063: 8 }").toAny() shouldBe mapOf("abc" to 8.0) // Multiple consecutive escapes
        JSON5.parse("{ id\\u0024ent: 9 }").toAny() shouldBe mapOf("id\$ent" to 9.0) // Escape resolves to $
        JSON5.parse("{ \\u005fid\\u005f: 10 }").toAny() shouldBe mapOf("_id_" to 10.0) // Escape resolves to _
        JSON5.parse("{ \\u0061: 11 }").toAny() shouldBe mapOf("a" to 11.0) // Identifier is a single escape
        JSON5.parse("{ \\u0061\\u0062c: 12 }").toAny() shouldBe mapOf("abc" to 12.0) // Starts with escapes, then normal char
    }
    ...
    fun `parse large hexadecimal numbers`() {
        ...
        JSON5.parse("0x1fffffffffffffff").toAny() shouldBe 2.305843009213694E18 // Might lose some precision
        // 0x2000000000000000 in decimal is 2305843009213693952
        JSON5.parse("0x2000000000000000").toAny() shouldBe 2.305843009213694E18 // Might be same as above due to double precision
        // A very large hex number
//        JSON5.parse("0x123456789abcdef123456789abcdef123456789abcdef").toAny() shouldBe 3.777995208190904E49
//        JSON5.parse("-0x123456789abcdef123456789abcdef123456789abcdef").toAny() shouldBe -3.777995208190904E49
        ...
        JSON5.parse("0x1FFFFFFFFFFFFF").toAny() shouldBe (2.0.pow(53.0) - 1)
        // 0x20000000000000 is 2^53
        JSON5.parse("0x20000000000000").toAny() shouldBe 2.0.pow(53.0)
        // One larger than 2^53 will not be exact
//        JSON5.parse("0x20000000000001").toAny() shouldBe (2.0.pow(53.0) + 2) // Due to rounding for doubles
    }
    ...
    fun `parse line continuations`() {
        JSON5.parse("'ab\\\ncd'").toAny() shouldBe "abcd"
        JSON5.parse("'ab\\\r\ncd'").toAny() shouldBe "abcd"
        JSON5.parse("'ab\\\rcd'").toAny() shouldBe "abcd" // \r is also a line terminator
        JSON5.parse("'ab\\\u2028cd'").toAny() shouldBe "abcd"
        JSON5.parse("'ab\\\u2029cd'").toAny() shouldBe "abcd"
    }
    ...
    fun `parse unrecognized simple escapes`() {
        JSON5.parse("'\\a'").toAny() shouldBe "a"
        JSON5.parse("'\\c'").toAny() shouldBe "c"
        JSON5.parse("'\\/'").toAny() shouldBe "/"
        JSON5.parse("'\\1'").toAny() shouldBe "1" // \1 is not an octal escape in JSON5
        JSON5.parse("'\\ '").toAny() shouldBe " " // \ followed by space
    }
}
